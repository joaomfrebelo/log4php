...
class LoggerConfiguratorDefault implements LoggerConfigurator
{
     ...
     *        the configuration.
     * @param mixed|null $input Either path to the config file or the
     *        configuration as an array. If not set, default configuration
     *        will be used.
     */
    public function configure(LoggerHierarchy $hierarchy, mixed $input = null)
    {
     ...
     * @return mixed configuration from the config file, as a PHP array.
     * @throws LoggerException If the configuration file cannot be loaded, or
     *        if the parsing fails.
     */
    private function parseFile(string $url): mixed
    {
    ...
    private function doConfigure(LoggerHierarchy $hierarchy, array $config)
    {
        ...
        if (isset($config['appenders']) && is_array($config['appenders'])) {
            foreach ($config['appenders'] as $name => $appenderConfig) {
                if (!is_array($appenderConfig)) {
                    $this->warn("Invalid configuration provided for appender");
                }
     ...
     */
    private function configureAppender(string $name, array $config)
    {
     ...
     * @param mixed $options
     */
    private function setOptions(mixed $object, mixed $options)
    {
