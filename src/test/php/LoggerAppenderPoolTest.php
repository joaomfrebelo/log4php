...
class LoggerAppenderPoolTest extends TestCase
{
    ...
    {
        $this->appenderMock = $this->getMockBuilder(
            LoggerAppenderConsole::class
        )->getMock();
    }
    ...
    {
        $this->expectError();
        $this->expectErrorMessage("log4php: Cannot add unnamed appender to pool.");
    ...
    /**
     * log4php: Appender [foo] already exists in pool. Overwriting existing appender.
     */
    public function testAppenderIsAdded()
    {
        $this->expectError();
        $this->expectErrorMessage("log4php: Appender [foo] already exists in pool. Overwriting existing appender.");
