 ...
 * @group appenders
 */
class LoggerAppenderPDOTest extends TestCase
{
    ...
    {
        $this->expectWarning();
        $this->expectWarningMessage("log4php: [LoggerAppenderPDO:myname]: Failed connecting to database. Closing appender. Error: PDO::__construct(): Argument #1 (\$dsn) must be a valid data source name");
        $dsn      = 'doenotexist';
        $appender = new LoggerAppenderPDO("myname");
        $appender->setDSN($dsn);
        $appender->activateOptions();
