...
class LoggerPatternConverterTest extends TestCase
{
     ...
     */
    private LoggerLoggingEvent $event;

    /**
     * Formatting info used with the logging event.
     */
    private LoggerFormattingInfo $info;
    ...
    /**
     * log4php: LoggerInvalidSuperglobalConverter: Cannot find superglobal variable $_FOO
     */
    public function testNonexistentSuperglobal()
    {
        $this->expectWarning();
        $this->expectWarningMessageMatches("/log4php: LoggerInvalidSuperglobalConverter/");
        $converter = new LoggerInvalidSuperglobalConverter($this->info);
        $converter->convert($this->event);
    }
